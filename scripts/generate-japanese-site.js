...
function localizeLinks(html){return html.replace(/https:\/\/hlctex\.com\/textile\//g,`${base}/ja/textile/`).replace(/href="\/textile\//g,'href="/ja/textile/').replace(/https:\/\/hlctex\.com\/company\//g,`${base}/ja/company/`).replace(/href="\/company\//g,'href="/ja/company/').replace(/href="\/"/g,'href="/ja/"').replace(/href="\/#/g,'href="/ja/#');}
...
function replaceVisible(html){const blocks=[]; html=html.replace(/<(script|style)\b[\s\S]*?<\/\1>/gi,b=>{blocks.push(b);return `__BLOCK_${blocks.length-1}__`;}); for(const [a,b] of ui)html=html.replace(new RegExp(esc(a),'g'),b); html=html.replace(/(\d+)\s+fabrics\b/gi,'$1種類'); html=html.replace(/>([^<]+)</g,(m,t)=>`>${replaceTerms(t).replace(/\band\b/gi,'・')}<`); return html.replace(/__BLOCK_(\d+)__/g,(_,i)=>blocks[+i]);}
function fixJson(html,route,product={}){return html.replace(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/gi,(whole,raw)=>{try{const d=JSON.parse(raw); const visit=x=>{if(!x||typeof x!=='object')return; if(['Product','CollectionPage','WebPage','Organization'].includes(x['@type']))x.inLanguage='ja-JP'; if(x['@type']==='Product'){
      ...
      if(Array.isArray(x.additionalProperty))x.additionalProperty.forEach(p=>{if(!p||typeof p!=='object')return; const names={'Yarn count':'糸番手','Fabric weight':'目付','Cuttable width':'有効幅','Construction':'組織','Finishing':'加工','Sample lead time':'サンプル納期','Bulk lead time':'量産納期','Applications':'用途','Test report':'試験報告書'}; if(names[p.name])p.name=names[p.name]; if(typeof p.value==='string')p.value=replaceTerms(clean(p.value)).replace(/\band\b/gi,'・');});
